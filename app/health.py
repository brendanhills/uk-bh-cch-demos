"""Standalone health check proxy for cch-demo Cloud Monitoring uptime checks.

Connects to the cch-demo WebSocket endpoint as a client and verifies
the model responds to a text query. Designed to run as a separate Cloud Run
service that monitors the main cch-demo service end-to-end:

    Cloud Monitoring -> GET /health/live -> this app -> WS -> cch-demo -> Live API

Endpoints:
    GET /health      - Basic reachability check (returns {"status": "ok"})
    GET /health/live - End-to-end model check via cch-demo WebSocket

Environment variables:
    CCH_DEMO_URL: WebSocket base URL of the cch-demo service
                  (default: ws://localhost:8000)

Local testing:
    # Start cch-demo on port 8000, then:
    CCH_DEMO_URL=ws://localhost:8000 \
    uv run --project .. uvicorn health:app --host 0.0.0.0 --port 8001

    curl http://localhost:8001/health/live

Deploy to Cloud Run:
    gcloud run deploy cch-demo-health \
      --source . \
      ...
      --set-env-vars CCH_DEMO_URL=wss://cch-demo-xxx.us-east1.run.app

Create uptime check:
    gcloud monitoring uptime create "cch-demo /health/live" \
      --resource-type=uptime-url \
      --resource-labels=host=cch-demo-health-xxx.us-east1.run.app,project_id=PROJECT \
      --protocol=https \
      --path=/health/live \
      --port=443 \
      --request-method=get \
...
async def health_live():
    """End-to-end health check that connects to cch-demo as a WebSocket client."""
    user_id = "uptime-check"
    session_id = f"health-{uuid.uuid4().hex[:12]}"
    ws_url = f"{CCH_DEMO_URL}/ws/{user_id}/{session_id}"
