...
- [ ] Task: Implement `tests/test_scenario_agents.py` to test each agent persona (Investigator, Policy Expert, Underwriter) using mocked tools but real scenario inputs to verify their reasoning logic.
    - [ ] *Sub-task*: Verify Investigator parallel tool execution logic.
    - [ ] *Sub-task*: Verify Underwriter synthesis logic (handling contradictory or missing data).
...
- [ ] Task: Update `loan_agent/sub_agents/policy_expert/tools.py` to ensure it extracts *citations* (Section #, Rule Name) as required by the "Explainability" goal in the prompt.
- [ ] Task: Verify the `Standard_Underwriting_Guidelines_2026.pdf` content supports the "Sarah Speed" auto-approval path AND the "Gary Escalate" manual review path.
...
- [ ] Task: *PRESENTATION TARGET*: Update Underwriter prompt to explicitly include the "Reasoning Chain" with citations in the `final_decision_output`.
...
- [ ] Task: Update `docs/PRESENTATION.md` to reflect the Gemini 3.1 architecture and "Thinking: HIGH" differentiator.
- [ ] Task: Map every demo scenario to a specific requirement in `demo_task.txt` (e.g., Sarah Speed = Requirement #4: Auto-approve in < 5 mins).
- [ ] Task: Perform a full 10-minute dry run using `docs/DEMO.md` and capture the exact sequence of agent "thoughts" and tool calls in the Streamlit UI.
- [ ] Task: Update `docs/DEMO.md` to explicitly mention the time-saving metrics (48h manual wait vs 10s agent execution).
- [ ] Task: Refresh architecture diagrams in `docs/diagrams/` to ensure they 100% match the consolidated `loan_agent/` structure.
