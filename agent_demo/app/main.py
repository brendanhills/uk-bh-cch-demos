...
async def generate_soap_note_endpoint(request: SOAPNoteRequest):
    """Generate structured clinical SOAP note for a session (#BUG-23)."""
    try:
        ...
        prompt = (
            "Please generate a complete, structured clinical SOAP note based on the following consultation transcript and medical records:\n\n"
            f"{conversation_text}"
        )

        soap_runner = Runner(app_name=APP_NAME, agent=soap_generator, session_service=session_service)
