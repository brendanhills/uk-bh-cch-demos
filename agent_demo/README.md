...
**Create the project from this sample** (replace `my-cch-demo` with your project name):

```bash
agents-cli create my-cch-demo -a adk@cch-demo
```
...
From the `src/cch-demo` directory, first change to the `app` subdirectory:
...
```bash
uv run --project .. uvicorn main:app --reload --host 127.0.0.1 --port 8000
```
...
uv run --project .. uvicorn main:app --host 127.0.0.1 --port 8000 > server.log 2>&1 &
...
```
ws://127.0.0.1:8000/ws/{user_id}/{session_id}
```
...
runner = Runner(app_name="cch-demo", agent=agent, session_service=session_service)
```
...
From the `src/cch-demo` directory, deploy with `gcloud run deploy`:

**Vertex AI Live API mode** (`GOOGLE_GENAI_USE_VERTEXAI=TRUE`):

```bash
gcloud run deploy cch-demo \
  --source . \
...
```bash
gcloud run services add-iam-policy-binding cch-demo \
  --project "${GOOGLE_CLOUD_PROJECT}" \
  --region "${GOOGLE_CLOUD_LOCATION}" \
  --member=allUsers \
...
```bash
gcloud run services describe cch-demo \
  --project "${GOOGLE_CLOUD_PROJECT}" \
  --region "${GOOGLE_CLOUD_LOCATION}" \
...
Agent Engine hosts **agents**, not web applications. It does not expose a public URL or serve static files — access is via authenticated Vertex AI SDK WebSocket connections only. This makes it well suited for backend agent hosting, but not for serving the full cch-demo webapp directly.
