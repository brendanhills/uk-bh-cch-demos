...
The Loan Approval Agent is a team of specialized AI agents that automates the loan underwriting process.

1. Investigator Agent: Gathers applicant data (Credit, Employment, Fraud).

2. Policy Expert Agent: Checks eligibility against policy documents.

3. Underwriter (Risk Analyst) Agent: Makes the final decision.

4. Loan Manager (Orchestrator): Coordinates the workflow.
...
Then the tests and evaluation can be run from the `loan-approval-agent` directory using
the `pytest` module:
...
agent-starter-pack create my-loan-approval-agent -a adk@loan-approval-agent
```
...
```bash
uvx agent-starter-pack create my-loan-approval-agent -a adk@loan-approval-agent
```
