...
class V2Provider:
    ...
    async def stream(self, audio_gen, channel_id=1, multi_channel=False, diarization=False, chunk_duration_sec=0.25):
        ...
        # 2. Setup bi-directional stream with automatic reconnection for 5-minute limits
        total_chunks_sent = 0
        stream_is_active = True
        
        while stream_is_active:
            async def request_generator():
                nonlocal total_chunks_sent
                yield cs_v2.StreamingRecognizeRequest(recognizer=self.recognizer_name, streaming_config=streaming_config)
                
                async for chunk in audio_gen:
                    total_chunks_sent += 1
                    self.current_audio_time = total_chunks_sent * chunk_duration_sec
            ...
            try:
                ...
                # If we exhausted the request_generator/audio_gen normally, terminate the loop
                stream_is_active = False

            except Exception as e:
                from google.api_core import exceptions
                if isinstance(e, (exceptions.Aborted, exceptions.ResourceExhausted)) or "duration of 5 minutes" in str(e).lower() or "aborted" in str(e).lower():
                    print(f"\n[STT Provider] Stream duration limit reached or stream aborted ({e}). Reconnecting seamlessly...")
