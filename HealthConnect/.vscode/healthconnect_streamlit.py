...
CREDENTIALS_PATH = "path/to/your/credentials.json" # Replace with the path to your service account credentials file
...
def run_healthconnect_app():
    ...
    if prompt := st.chat_input("Type your message..."):
        ...
        try:
            response = get_dialogflow_response(
                prompt,
                session_id=st.session_state.session_id,  # Use the session ID
    ...
    if "session_id" not in st.session_state:
        st.session_state.session_id = "healthconnect_user_" + str(st.session_id) # Generate a unique session ID.
