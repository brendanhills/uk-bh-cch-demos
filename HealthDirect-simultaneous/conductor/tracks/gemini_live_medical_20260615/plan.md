...
- [x] Task: Build Responsive Side-by-Side Web UI
    - [x] Design a premium browser interface with side-by-side dual columns: Patient/Parent (Target Language) vs. Nurse (English).
    - [x] Implement custom styles matching premium modern web aesthetics.
    - [x] Write frontend unit tests for WebSocket client connectivity.
- [x] Task: Integration of Microphone & Web Audio Out
    - [x] Integrate local microphone capture and secure WebSocket streaming.
    - [x] Implement low-latency audio playback of Gemini's TTS translations.
- [x] Task: Conductor - User Manual Verification 'Phase 3: Frontend Web App' (Protocol in workflow.md)

## Phase 4: Verification and Evaluation (Simulation Mode)
- [x] Task: Simulation Evaluation Script
    - [x] Create a simulation script (`simulate_medical_call.py`) that reads a pre-recorded nurse-patient bilingual audio dialogue.
    - [x] Feed dialogue to the Live API and render the output side-by-side in the console.
    - [x] Verify that Australian terminology and custom glossary terms are correctly chosen in real-time outputs.
- [x] Task: Conductor - User Manual Verification 'Phase 4: Evaluation & Handoff' (Protocol in workflow.md)
